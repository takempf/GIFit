...
import type { VideoAdapter, GifAdapter, StorageAdapter, GifProgressCallbacks } from './types';
import type { GifConfig, VideoMetadata, ExtensionMessage } from '@/types';
...
export class ExtensionVideoAdapter implements VideoAdapter {
  ...
  async getMetadata(): Promise<VideoMetadata | null> {
    return (await sendMessageToActiveTab('GET_VIDEO_METADATA')) as VideoMetadata | null;
  }

  async captureFrame(): Promise<string | null> {
    return (await sendMessageToActiveTab('CAPTURE_VISIBLE_FRAME')) as string | null;
  }
}
...
export function setupPopupPort(): () => void {
  ...
  return () => {
    portPromise.then((p) => p?.disconnect());
  };
}
